...
public function GetSubCategoryByParent_id($parent_id)
{
    $sql="SELECT id_category,name_category from category where parent_id=$parent_id";
...
//function get data post by id user^^
public function GetPostConfessionUser($id,$start=0,$limit=5,$search="")
{
    // lấy tất có các bài viết người đang dùng tài khoản
    if($search==""){
        $sql="SELECT id, UserName, Title,Content,view,DateUp,id_post,Avatar FROM post where id=$id  LIMIT $start, $limit";
    }
    else{
        $sql="SELECT id, UserName, Title,Content,view,DateUp,id_post,Avatar FROM post where id=$id and UserName like '%$search%' Or Title like '%$search%'  LIMIT $start, $limit";  
    }
...
public function UpdatePostbyIdUser($id,$avatar)
{
    // lấy tất có các bài viết người đang dùng tài khoản
    
    $sql="UPDATE post SET Avatar = '$avatar'  WHERE  id=$id";
    $result = $this->conn->query($sql);
